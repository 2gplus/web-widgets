...
export function useGridState(
    ...
    const memoizedGridActions = useMemo<Grid.Actions>(() => {
        return {
            ...
            setRemoteExecuting: payload => dispatch({ type: "setRemoteExecuting", payload })
        };
    }, [dispatch]);
...
function gridStateReducer(state: Grid.State, action: Action): Grid.State {
    switch (action.type) {
        ...
        case "setRemoteExecuting":
            return {
                ...state,
                executingRemoteSort: action.payload
            };
    }
}
