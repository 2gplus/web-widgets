...
import { AlignmentEnum, ColumnsPreviewType, DataObjectsType, WidthEnum } from "../../typings/DatagridProps";
    ...
    const renderCell = useCallback(
        (column: ColumnProperty, value: T, rowIndex: number) =>
            visibleColumns.find(c => c.id === column.id) || preview
                ? cellRenderer(
                      (children, className) => {
                          const onClick = props.rowOnClickHandler;
                          ...
                          return (
                              <div
                                  ...
                                  onClick={(e: any) => {
                                      if (onClick) {
                                          onClick(e, false, value);
                                      }
                                  }}
                                  onDoubleClick={(e: any) => {
                                      if (onClick) {
                                          onClick(e, true, value);
                                      }
                                  }}
                                  onKeyDown={(e: any) => {
        ...
        >
            <div className="table" role="table">
                <TableHeader
                    ...
                    pagination={pagination ? { pagination, position: props.pagingPosition } : undefined}
                    widgets={gridHeaderWidgets}
                    ...
                    {rows.map((row, rowIndex) => {
                        ...
                        if (props.dataAttributes) {
                            for (const dataObject of props.dataAttributes) {
                                fragmentProps[`data-${dataObject.attribute}`] = dataObject.data?.get(row.item)?.value;
                            }
                        }
                        return (
                            <div
